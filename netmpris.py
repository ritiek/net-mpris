# This code is mainly stolen from https://github.com/mps-youtube/mps-youtube
# with a few modifications to run it as a standalone module
...
mprisctl = MprisConnection(conn1)
t = multiprocessing.Process(target=main, args=(conn2,))
t.daemon = True
t.start()

# import tempfile
# sockpath = tempfile.mktemp('.sock', 'mpris-test')
# mpv_usesock = '--input-ipc-server'
# print(sockpath)

# metadata = ('XpmeVNxZ-Ks', 'ILLENIUM, Jon Bellion - Good Things Fall Apart', 240, 'https://i.ytimg.com/vi/XpmeVNxZ-Ks/default.jpg', [''], '')
# mprisctl.send(('metadata', metadata))
# mprisctl.send(('pause', False))
# mprisctl.send(('stop', False))
# # mprisctl.send(('volume', 10))
# mprisctl.send(('time-pos', 60))
# # control mpv?
...
def on_metadata(player, metadata):
    ...
    print(mpris_metadata)
    mprisctl.send(('metadata', mpris_metadata))
...
def on_pause(player, status):
    mprisctl.send(('pause', True))
    print('Paused the song: {}'.format(player.get_title()))


def on_stop(player, status):
